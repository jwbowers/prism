// Package cli provides interactive onboarding wizard for Prism.
package cli
...
// InitWizard provides interactive first-time setup for Prism
...
func (iw *InitWizard) showWelcome() {
	fmt.Printf("\n🎉 %s\n", color.CyanString("Welcome to Prism!"))
	fmt.Println()
...
func (iw *InitWizard) stepAWSConfiguration() error {
	...
	if _, err := os.Stat(credentialsPath); err == nil {
		...
		fmt.Println()
		fmt.Println("Prism requires AWS credentials to launch workspaces.")
		fmt.Println()
...
func (iw *InitWizard) stepBudget() float64 {
	...
	fmt.Println("Prism will notify you but won't automatically stop workspaces.")
	fmt.Println()

	if !iw.promptYesNo("Would you like to set a monthly budget?", false) {
		fmt.Println("⏭️  Skipping budget setup (you can configure this later with 'prism budget')")
		fmt.Println()
		return 0
	}
...
func (iw *InitWizard) stepHibernation(recommendedPolicy string) string {
	...
	for {
		...
		case "1", "recommended", "yes", "y":
			...
			fmt.Printf("💡 You can adjust this later with 'prism idle profile'\n\n")
...
func (iw *InitWizard) stepTemplateRecommendations(area *ResearchArea) {
	...
	fmt.Println("   You can explore all templates with: prism templates list")
	fmt.Println()
}
...
func (iw *InitWizard) saveInitialization(area *ResearchArea, budget float64, policy string) error {
	...
	initFile := fmt.Sprintf("%s/.prism/.initialized", homeDir)
...
func (iw *InitWizard) showCompletion() {
	...
	fmt.Println()
	fmt.Printf("  %s\n", color.CyanString("prism launch \"Python Machine Learning (Simplified)\" my-first-project"))
	fmt.Println()
	fmt.Println("Other helpful commands:")
	fmt.Printf("  • %s - List all available templates\n", color.New(color.FgHiBlack).Sprint("prism templates list"))
	fmt.Printf("  • %s - List your workspaces\n", color.New(color.FgHiBlack).Sprint("prism workspace list"))
	fmt.Printf("  • %s - Connect to a workspace\n", color.New(color.FgHiBlack).Sprint("prism workspace connect <name>"))
	fmt.Printf("  • %s - Launch interactive interface\n", color.New(color.FgHiBlack).Sprint("prism tui"))
	fmt.Println()
	fmt.Println("📚 Documentation: https://github.com/scttfrdmn/prism")
	fmt.Println()
}
